...
--END
GO
ALTER PROCEDURE [dbo].[P_getTickets]
 @userId int=null,
 @pageIndex INT,
 @pageSize INT
AS
...
(@userId is null or t.UserId = @userId)
 ...
 select * from  (select Row_Number() OVER(order by t.code desc )AS RowNum , t.*,u.name as 'TicketName',u.introduce from Tickets t left join AdmissionTicket u on t.AdmissionTicketId=u.id where
(@userId is null or t.UserId = @userId)
)t
where t.RowNum > (@pageIndex -1) * @pageSize 
         AND t.RowNum <=   @pageIndex * @pageSize
END
GO
alter PROCEDURE [dbo].[P_getAccountUsers]
 @Mobile nvarchar(50)=null,
 @DisplayName nvarchar(50)=null,
 @babyName nvarchar(50)=null,
...
(@Mobile is null or t.Mobile = @Mobile)
and (@DisplayName is null or t.DisplayName like '%' + @DisplayName + '%')
and (@babyName is null or t.babyName like '%' + @babyName + '%')
